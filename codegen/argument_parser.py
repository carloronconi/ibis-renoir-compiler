...
class ArgumentParser:
    ...
    def parse(self, operand: Node) -> str:
        ...
        if isinstance(operand, ibis.expr.operations.generic.TableColumn):
            ...
            if operand.dtype.name == "String" and self.other_is_date:
                return f"NaiveDate::parse_from_str(\"{operand.value}\", \"%Y-%m-%d\").unwrap()"
            if operand.dtype.name == "String":
                return f"\"{operand.value}\""
            return operand.name
        elif isinstance(operand, ops.logical.Comparison):
            self.other_is_date = operand.left.dtype.name == "Date" or operand.right.dtype.name == "Date"
